    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useLocalStorage</Badge>
          로컬 스토리지
        </CardTitle>
        <CardDescription>브라우저를 새로고침해도 값이 유지됩니다.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          />
          {name && <p className="text-sm text-muted-foreground">저장됨: <strong>{name}</strong></p>}
        </div>
        <div className="flex items-center gap-3">
          ...
          />
          <Label>테마: <strong>{theme === "dark" ? "다크" : "라이트"}</strong></Label>
        </div>
      </CardContent>
    </Card>
  );
...
function DebounceDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useDebounce</Badge>
          디바운스
        </CardTitle>
        <CardDescription>입력 후 500ms 뒤에 디바운스 값이 업데이트됩니다.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="rounded-md bg-muted/50 p-3 space-y-1 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-primary">{debouncedValue || "—"}</span>
          </div>
        </div>
      </CardContent>
    </Card>
...
function ToggleDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useToggle</Badge>
          토글
        </CardTitle>
...
function CounterDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useCounter</Badge>
          카운터
        </CardTitle>
        <CardDescription>증가/감소/초기화 기능이 내장된 카운터입니다.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
...
function CopyToClipboardDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useCopyToClipboard</Badge>
          클립보드 복사
        </CardTitle>
        <CardDescription>클릭 시 텍스트를 클립보드에 복사합니다.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {snippets.map((s) => (
          ...
          >
            <div className="min-w-0">
              ...
              <p className="truncate text-sm font-mono">{s.value}</p>
            </div>
            <Button
              size="icon"
              variant="ghost"
        ...
        {copiedText && (
          <p className="text-xs text-muted-foreground">
            복사됨: <span className="font-mono text-primary">{copiedText}</span>
          </p>
        )}
      </CardContent>
    </Card>
...
function WindowSizeDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useWindowSize</Badge>
          창 크기
        </CardTitle>
        <CardDescription>현재 브라우저 창의 크기를 실시간으로 표시합니다.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-center justify-center gap-6">
          <div className="text-center">
            <Monitor className="mx-auto mb-2 size-8 text-primary" />
            <div className="flex gap-3">
              <div className="rounded-md bg-muted/50 px-4 py-3 text-center">
                <p className="text-xs text-muted-foreground">너비</p>
                <p className="text-2xl font-bold tabular-nums text-primary">{width}</p>
                <p className="text-xs text-muted-foreground">px</p>
              </div>
              <div className="rounded-md bg-muted/50 px-4 py-3 text-center">
                <p className="text-xs text-muted-foreground">높이</p>
                <p className="text-2xl font-bold tabular-nums text-primary">{height}</p>
                <p className="text-xs text-muted-foreground">px</p>
              </div>
            </div>
          </div>
...
function MediaQueryDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useMediaQuery</Badge>
          미디어 쿼리
        </CardTitle>
        <CardDescription>현재 활성화된 브레이크포인트를 감지합니다.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          {breakpoints.map((bp) => (
...
function HoverDemo() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Badge variant="outline" className="font-mono text-xs">useHover</Badge>
          호버 감지
        </CardTitle>
        <CardDescription>요소에 마우스를 올리면 상태가 변경됩니다.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div
          ref={ref1}
